...
const DAUM_KAKAO_API_JS_KEY = "773de57cfd5f602750cdfb9ed7ffdbf2";
...
class Map {
  ...
  async mount(elementId) {
    ...
    if (Map.cachedMaps[elementId]) {
      ...
      this.map = new Map.daum.maps.Map(document.getElementById(elementId), {
        center: new Map.daum.maps.LatLng(35.65919598808177, 127.73894402702498),
        level: 13
      });
      this.map.setCopyrightPosition(
        Map.daum.maps.CopyrightPosition.BOTTOMRIGHT,
        true
      );
      this.map.clusters = {};
  ...
  addMarkerClusters(clusterSpecs = []) {
    clusterSpecs.forEach(({ key, color, zIndex = 0, singleIconURL = null }) => {
      ...
      const cluster = (this.map.clusters[
        key
      ] = new Map.daum.maps.MarkerClusterer({
        map: this.map,
        averageCenter: true,
        gridSize: 30,
        minClusterSize: 2,
  ...
  addMarkers(markerSpecs = []) {
    ...
    markerSpecsWithoutClusterKey.forEach(
      ({ lat, lng, title = null, onClick = null }) => {
  ...
  setCenter({ lat, lng, maxLevel = 8 }) {
    ...
    this.map.panTo(new Map.daum.maps.LatLng(lat, lng));
  }
}
